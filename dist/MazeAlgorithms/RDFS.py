...
class Cell(RDFS):
    def __init__(self, window, width, height, size, visitColour, wallColour, x, y, currentColour, maze_x, maze_y, mazeColour):
        ...
        super().__init__(window, width, height, size, visitColour, wallColour, x, y, currentColour, maze_x, maze_y, mazeColour)
        self.visited = False
        
    def draw(self): # Function for drawing one cell in the maze
        x, y =  self.maze_x + self.x * self.size, self.maze_y + self.y * self.size # Grid position to screen position
...
class MazeGenerator(RDFS):
    ...
    def draw_grid(self):
        # Draw out the current maze by drawing each cell in the grid
