...
def draw_objects(window, width, height, font, mazeSize, sizeBorderColour, themeBorderColour, themeText, speedBorderColour, speedText, pauseText, pauseBorderColour, pauseBgColour):
    ...
    size_x, size_y = pos(width, height, 30.74, 19.5, size_w, size_h)
    ...
    return back, gen, maze_size, theme, speed, pause

def gen_maze(window, width, height, size, visitColour, wallColour, x, y, currentColour, maze_x, maze_y, mazeColour, scale, fin_x, fin_y):
     # Initialise the maze generator attributes and methods
    maze = MazeGenerator(window, width, height, size, visitColour, wallColour, x, y, currentColour, maze_x, maze_y, mazeColour, scale, fin_x, fin_y)
    return maze # run one step of the maze generation process

def gen_grid(window, width, height, size, visitColour, wallColour, x, y, currentColour, maze_x, maze_y, mazeColour, scale, fin_x, fin_y):
     # Initialise the maze generator attributes and methods
    grid = MazeGenerator(window, width, height, size, visitColour, wallColour, x, y, currentColour, maze_x, maze_y, mazeColour, scale, fin_x, fin_y)
...
class Generate:
    ...
    def run(self):
        ...
        self.back_button, self.gen_button, self.maze_size_button, self.theme_button, self.speed_button, self.pause_button = draw_objects(self.window, self.width, self.height, self.font, self.mazeSize, self.sizeBorderColour, self.themeBorderColour, self.themeText, self.speedBorderColour, self.speedText, self.pauseText, self.pauseBorderColour, self.pauseBgColour)
        ...
        if not self.paused:
            if self.maze_generated and self.maze: # If a maze has been generated
                ...
                # Create a new maze
                self.maze = gen_maze(self.window, self.maze_w, self.maze_h, self.size, self.visitColour, self.wallColour, self.x, self.y, self.currentColour, self.maze_x, self.maze_y, self.mazeColour, self.scale, self.fin_x, self.fin_y)
                self.maze.run()
            ...
            self.maze.done() # Redraw the checkered box
            
        if self.maze is None: # If there is no maze drawn
            # Create an empty grid (Updates each cycle to keep up with any changes made)
            self.grid = gen_grid(self.window, self.maze_w, self.maze_h, self.size, self.visitColour, self.wallColour, self.x, self.y, self.currentColour, self.maze_x, self.maze_y, self.mazeColour, self.scale, self.fin_x, self.fin_y)
    ...
    def click(self, pos):
        if self.back_clicked:
            ...
            if self.theme == "Original":
                ...
                self.themeText = "Red"
                self.mazeColour, self.visitColour, self.wallColour, self.currentColour = Colours.BLACK, Colours.RED, Colours.NEONBLUE, Colours.NEONBLUE
